...
class Events extends Component {
  ...
  getActiveRegistrationItem = () => {
    const {activeEventId} = this.state
    const activeRegistrationStatusDetails = eventsList.find(
      eachItem => eachItem.id === activeEventId,
    )
    if (activeRegistrationStatusDetails) {
      return activeRegistrationStatusDetails.registrationStatus
    }
    return ''
  }
      ...
      <div className="bg-container">
        ...
        <div className="event-status-container">
          <ActiveEventRegistrationDetails
            activeEventRegistrationStatus={this.getActiveRegistrationItem()}
          />
        </div>
      </div>
    )
